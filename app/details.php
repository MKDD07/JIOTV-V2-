...
// Error reporting should be handled during development, remove for production.
error_reporting(E_ALL);
ini_set('display_errors', 1);

// Data retrieval from request
$data = $_REQUEST["data"] ?? "";
...
<body>
    ...
    <div class="channel_logo" style="text-align: center;">
        <img src="<?php echo $image; ?>" alt="Logo" width="100px" height="auto" gap:20px; style="margin: 0 auto 0 50px; display: block;" />
        <div id="player-container"></div>
    </div>
    <br />
            <?php
            // Encode data for live button link
            $cp_link_live = $cid . '=?=' . $id;
            ...
            <button id="refreshButton">
                <img src="assets/main_assets/reload.svg" alt="Refresh" width="24" height="24" />
            </button>

           <!-- <button id="logoutButton">
            <img src="assets/main_assets/logout.svg" alt="Logout" width="24" height="24" />
          </button>
            <a href="<?php echo "$jio_path/play.php?data=$cp_link_live"; ?>" class="fsButton">
    <img src="assets/main_assets/fullscreen.svg" alt="Fullscreen Icon" width="24" height="24" />
</a> -->
    ...
    <?php
// Checking if catchup is available
if ($c == "y") {
    $cp_link_live = $cid . '=?=' . $id;
    $cp_link_live = base64_encode($cp_link_live);
    ?>
    <div class="card-container">
        <div class="grouped-elements" style="display: inline-block; position: relative; border: 1px solid #BA3B46; border-radius: 8px;">
            <a class="btn btn-danger" style="background-color: #BA3B46;color:#fff; padding: 9px 21px; height: 40px; text-decoration: none; cursor: default;">CATCHUP</a>
        </div>
        <div class="scroll-container">
            ...
            for ($i = 0; $i >= -7; $i--) {
                ...
                ?>
                <div class="date-card">
                    <img class="card-img-top" src="<?php echo $image; ?>" alt="logo" width="20px">
                    <a href="<?php echo "$jio_path/catchup/cp.php?data=$cp_link"; ?>" class="btn btn-primary"><?php echo $prevDate->format('d-m-Y'); ?></a>
                </div>
            <?php } ?>
        </div>
    </div>
...
.grouped-elements:hover .circle {
  display: none; /* Hide circle on hover */
    ...
    .card-img-top:active, .card-img-top:focus{
...
.card-container{
  ...
  .logout-button {
    ...
    .logout-button img {
        display: block; /* Ensures SVG fills its container */
        width: 100%; /* Ensures SVG scales properly */
    }
    <style>
        button, .fsButton {
            background: none;
        ...
        #logoutButton img, #refreshButton img, .fsButton img {
            width: 24px;
            height: 24px;
...
.jw-svg-icon:active {
    border: none;
    outline: none; /* To remove any default outline that might be applied */
}
...
document.addEventListener('DOMContentLoaded', function() {
    // Get reference to the JW Player element
    const player = jwplayer('myElement');

    // Function to update background gradient based on video content
    function updateBackgroundGradient() {
        // Get the current video frame from the player
        const videoFrame = player.getPlaylistItem(player.getPlaylistIndex());

        // Extract dominant color from the video frame or use a default color
        const dominantColor = getDominantColor(videoFrame.image || 'default.jpg');

        // Update JW Player container background gradient with the dominant color
        document.getElementById('jw-container').style.background = `linear-gradient(to bottom, ${dominantColor}, rgba(0, 0, 0, 0.8))`;
    }

    // Function to get dominant color from an image (You need to implement this)
    function getDominantColor(imageUrl) {
        // Implement logic to extract dominant color from the image
        // You can use libraries like ColorThief or implement your own algorithm
        // For demonstration, returning a default color
        return '#000000';
    }

    // Call the function to update background gradient when the video changes
    player.on('playlistItem', updateBackgroundGradient);

    // Initial call to set the background gradient when the page loads
